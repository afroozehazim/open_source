...
void sse_decode(const int64_t* digits, uint8_t fac_idx, uint8_t exp_idx, double* out_p) {
	...
	double   frac10     = alp::frac_arr[exp_idx];
...
inline void decode(const uint64_t* encoded_integers, uint8_t fac_idx, uint8_t exp_idx, double* output) {
	for (size_t i {0}; i < ALP_VECTOR_SIZE; i++) {
		auto encoded_integer = static_cast<int64_t>(encoded_integers[i]);
		output[i] = alp::decode_value(encoded_integer, fac_idx, exp_idx);
	}
}
