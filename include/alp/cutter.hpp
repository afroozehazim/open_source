 ...
 */
inline double estimate_compression_size(uint8_t right_bit_width, uint8_t left_bit_width, uint16_t exceptions_count,
										uint64_t sample_count) {
	double exceptions_size = exceptions_count * (rd_exception_position_size + rd_exception_size);
	double estimated_size = right_bit_width + left_bit_width + (exceptions_size / sample_count);
	return estimated_size;
}

template <bool PERSIST_DICT>
inline double build_left_parts_dictionary(const double* in_p, uint8_t right_bit_width, state& stt) {
	...
	std::sort(left_parts_sorted_repetitions.begin(), left_parts_sorted_repetitions.end(), 
		[](const std::pair<uint16_t, uint64_t>& a, const std::pair<uint16_t, uint64_t>& b){ return a.first > b.first; });

	// Exceptions are left parts which do not fit in the fixed dictionary size
	uint32_t exceptions_count {0};
	for (size_t i {MAX_RD_DICTIONARY_SIZE}; i < left_parts_sorted_repetitions.size(); i++) {
	...
	uint8_t actual_dictionary_size = std::min<uint64_t>(MAX_RD_DICTIONARY_SIZE, left_parts_sorted_repetitions.size());
	uint8_t left_bit_width = std::max<uint8_t>(1, std::ceil(std::log2(actual_dictionary_size)));
	...
	double estimated_size = estimate_compression_size(right_bit_width, left_bit_width, exceptions_count, stt.sampled_values_n);
	return estimated_size;
}

inline void find_best_dictionary(double* smp_arr, state& stt) {
	uint8_t right_bit_width;
	double best_dict_size = std::numeric_limits<int64_t>::max();
	// Finding the best position to CUT the values
	for (size_t i {1}; i <= CUTTING_LIMIT; i++) {
		uint8_t candidate_right_bit_width = 64 - i;
		double estimated_size = build_left_parts_dictionary<false>(smp_arr, candidate_right_bit_width, stt);
	...
	double estimated_size = build_left_parts_dictionary<true>(smp_arr, right_bit_width, stt);
}
