...
namespace alp {
    ...
    inline double build_left_parts_dictionary(const double *in_p, uint8_t right_bit_width, state &stt) {
        ...
        uint8_t actual_dictionary_size = std::min<uint64_t>(MAX_RD_DICTIONARY_SIZE,
                                                            left_parts_sorted_repetitions.size());
        ...
        double estimated_size = estimate_compression_size(right_bit_width, left_bit_width, exceptions_count,
    ...
    inline void rd_init(double *data_column, size_t column_offset, size_t tuples_count, double *sample_arr,
                        state &stt) {
