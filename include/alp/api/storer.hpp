...
namespace alp { namespace storer {
    ...
    struct MemStorer {
        ...
        MemStorer(){}
        MemStorer(uint8_t* out_buffer) : out_buffer(out_buffer), buffer_offset(0){}

        void set_buffer(uint8_t* out){
            out_buffer = out;
        }
        ...
        void store(void* in, size_t bytes_to_store){
            memcpy((void*) (out_buffer + buffer_offset), in, bytes_to_store);
    ...
    struct MemReader {
        ...
        MemReader(){}
        MemReader(uint8_t* in_buffer) : in_buffer(in_buffer), buffer_offset(0){}

        void set_buffer(uint8_t* in){
            in_buffer = in;
        }
