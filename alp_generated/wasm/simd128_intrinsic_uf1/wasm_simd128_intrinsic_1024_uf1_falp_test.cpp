...
class wasm_simd128_intrinsic_1024_uf1_falp: public ::testing::Test
{
	public:
	   double * dbl_arr;
	   double * exc_arr;
	...
	~wasm_simd128_intrinsic_1024_uf1_falp () override
	{
		delete[] dbl_arr;
		delete[] exc_arr;
		delete[] pos_arr;
...
;
TEST_F(wasm_simd128_intrinsic_1024_uf1_falp, fused)
{
	for (auto & dataset : alp_bench::datasets)
	{
		...
		// Init
		alp::init(dbl_arr, 0, 1024, smp_arr, stt);
		// Encode
		alp::encode(dbl_arr, exc_arr, pos_arr, exc_c_arr, dig_arr, stt);
		alp::analyze_ffor(dig_arr, bw, base_arr);
		alp::generated::ffor::fallback::scalar::ffor(dig_arr, ffor_arr, bw, base_arr);
		// Decode
		generated::falp::wasm::simd128::falp(reinterpret_cast < uint64_t * > (ffor_arr), dec_dbl_arr, bw, reinterpret_cast < uint64_t * > (base_arr),stt.fac, stt.exp);
...
TEST_F(wasm_simd128_intrinsic_1024_uf1_falp, unfused)
{
	for (auto & dataset : alp_bench::datasets)
	{
		std:: ifstream         ifile(dataset.sample_csv_file_path, std::ios:: in );
