...
class x86_64_sse_intrinsic_1024_uf1_falp: public ::testing::Test
{
	public:
	   double * dbl_arr;
	   double * exc_arr;
	...
	~x86_64_sse_intrinsic_1024_uf1_falp () override
	{
		delete[] dbl_arr;
		delete[] exc_arr;
		delete[] pos_arr;
...
;
TEST_F(x86_64_sse_intrinsic_1024_uf1_falp, fused)
{
	for (auto & dataset : alp_bench::datasets)
	{
		...
		// Decode
		generated::falp::x86_64::sse::falp(reinterpret_cast < uint64_t * > (ffor_arr), dec_dbl_arr, bw, reinterpret_cast < uint64_t * > (base_arr),stt.fac, stt.exp);
...
TEST_F(x86_64_sse_intrinsic_1024_uf1_falp, unfused)
{
	for (auto & dataset : alp_bench::datasets)
	{
		std:: ifstream         ifile(dataset.sample_csv_file_path, std::ios:: in );
